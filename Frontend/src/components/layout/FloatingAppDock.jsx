...
export default function FloatingAppDock({ onOpenCompanion, standalone = true, className = "" }) {
  const button = (
    <motion.button
      whileHover={{ scale: 1.06, y: -2 }}
      whileTap={{ scale: 0.94 }}
      onClick={onOpenCompanion}
      className={`group relative flex items-center gap-2.5 bg-olive-900 text-cream px-4 sm:px-5 py-3 rounded-full shadow-2xl border border-olive-700/50 backdrop-blur transition-all pointer-events-auto cursor-pointer ${className}`}
